...
parser.add_argument('test', type=int, help='1: gcorr, 2: gcorr dcsbm, 3: pearson')
...
parser.add_argument('pooled_variance', type=int, nargs='?', default=1, 
    help='(for gcorr dcsbm) whether to use the pooled variance test statistics')
parser.add_argument('Z', type=int, nargs='?', default=1, 
    help='(for gcorr) 1: given true assignments, 2: used estimated assignments (with optimally chosen number of K), 3: estimate assignments with fixed K')
...
def format_output_path(args):
    ...
    if args.test == 2:
        if not args.pooled_variance:
            output_path += '_noPooledVariance'
        ...
        for j in range(i + 1, num_graphs):
            ...
            if args.test == 1:
                ...
                pvalues[i, j] = dcsbm_pvalue(G1, G2, min_comm=min_comm, max_comm=max_comm, epsilon1=epsilon1, epsilon2=epsilon2,
                    pooled_variance=args.pooled_variance, num_perm=args.num_iter)
            elif args.test == 3:
